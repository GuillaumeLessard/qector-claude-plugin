...
1. **Step 1 - Diagnostic Audit (Dry-Run)**:
   - Run `python scripts/qector_system_setup.py --check-only`.
   - Inspect:
   ...
   - Do not call the MCP `system_setup` tool unless `qector-admin` is enabled
     with `QECTOR_ADMIN_ENABLED=1`. Prefer the CLI.
...
2. **Step 2 - User Approbation & Execution**:
   ...
   - Upon confirmation, run `python scripts/qector_system_setup.py --confirm`.
