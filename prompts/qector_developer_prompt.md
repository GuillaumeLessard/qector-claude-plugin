...
1. **Library-first**: `python -m pip install -r requirements.txt`
   ...
   `qector-bench` (25 Provisional companion tools for DEM,
...
- Drop-ins: `qector_decoder_v3.pymatching_compat.Matching`
  ...
  points. The bench server's `qector-bench.sinter_decoder_list`
  and `qector-bench.pymatching_compat_check` are the live
  probes for these.

### 3. DEM / circuit integration (manual 14)

- Bench: `qector-bench.dem_inspect` parses a minimal Stim-style
  DEM; `qector-bench.dem_collapse_parallel` applies the manual
...
- Check the environment with `qector-library.compat_report`
  or `qector-bench.env_block`. Optional Workbench diagnostics
  require target-device `tools/list` negotiation.
- For CUDA / OpenCL / tier questions, use
  `qector-bench.hardware_probe` and
  `qector-bench.license_active_check` - never hard-code a
  tier.
