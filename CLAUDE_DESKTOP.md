...
2. Merge the `qector-library` server from [`mcp/claude_desktop_config.json`](mcp/claude_desktop_config.json) into the desktop app MCP configuration.
3. Replace `<PLUGIN_ROOT>` with the package directory on the target machine. This token is never replaced in the repository.
