...
This module lives at ``python/qector_math_ground_truth.py`` (not repo root).
``conftest.py`` adds ``python/`` to ``sys.path`` for pytest; standalone
scripts do the same explicitly (see ``scripts/run_threshold_sweep.py``).
"""
