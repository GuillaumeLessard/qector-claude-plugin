...
description: >-
  ...
  MCP server (25 Provisional tools) for the math and DEM
...
1. **Ground your session. This works APP-FREE**: prefer the
   ...
   `qector-bench.code_family_info` returns the live structure
...
2. **Understand the problem before measuring**. Decode
   ...
   `qector-bench.code_family_info`) before launching production
...
3. **Choose measurement by question**:
   ...
   - "did the correction satisfy the syndrome?" ->
     `qector-bench.decode_faithfulness_check` (Theorem 1
     external verifier).
   - "what is the per-shot latency distribution on this
     machine?" -> `qector-bench.hot_path_microbench` (never
     a portable claim).
...
- GPU / GNN availability is device- and license-gated. Read
  the live `qector-bench.license_active_check` /
  `qector-bench.hardware_probe` response instead of assuming
  a tier.
- Every delivered result must include the syndrome-validation
  ...
  `qector-bench.decode_faithfulness_check` is the external
  verifier.
...
- Library bench: `qector-bench.dem_inspect` parses a
...
`qector-bench.artifacts_sha256` are the chapter 22.3
